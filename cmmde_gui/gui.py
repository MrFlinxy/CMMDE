...
def cmmde_gui():
    ...
    editor = JSMEEditor(
        value=" ", height=500, format="smiles", subscriptions=["smiles"]
    )
    ...
    TextArea = pn.widgets.TextAreaInput(
        value="Computational Molecular and Material Design Environment\n Authors:\n Universitas Pertamina\n Institut Teknologi Sumatera\n Institut Teknologi Bandung\n Masyarakat Komputasi Indonesia\n\nSupported by:\n Konsorsium Pengembangan Sains Komputasi\n",
        height=500,
        disabled=True,
    )
    # CMMDE software options
    software_main = pn.widgets.Select(
        name="Software selections",
        value="Orca",
        options=["Orca", "XTB", "Dcdftbmd", "Quantum Espresso", "GROMACS"],
    )
    software = {
        "Orca": "orca",
        "GROMACS": "gromacs",
    ...
    # Orca card
    job_orca = pn.widgets.Select(
        name="Job selections",
        value=["Single point calculation"],
        options=[
    ...
    method_orca = pn.widgets.Select(
        name="Method selections",
        value="GFN2-xTB",
        options=["GFN2-xTB", "GFN1-xTB", "B3LYP/def2-svp", "M06/def2-svp"],
    )
    method_orca_dict = {
        "GFN2-xTB": "XTB2",
        "GFN1-xTB": "XTB1",
        "DFTB2": "DFTB2",
        "DFTB2-gammah": "DFTB2_gammah",
        "DFTB3": "DFTB3",
        "DFTB3-diag": "DFTB3-diag",
        "B3LYP/def2-svp": "B3LYP def2-svp",
        "M06/def2-svp": "M06 def2-svp",
    }
    dispersion_cor = pn.widgets.Select(
        name="Dispersion corrections",
        value="None",
        options=["None", "D3", "D3BJ", "D4"],
    )
    solvent = pn.widgets.Select(
        name="Solvent",
        value="None",
        options=[
            "None",
    ...
    def run_orca(event):
        ...
        cmd = subprocess.run(
            [
                ...
                "{}".format(jobs),
                "-m",
                "{}".format(method[method_orca.value]),
                "-c",
                "{}".format(charge.value),
                "-mult",
                "{}".format(mult.value),
    ...
    def run(event):
        ...
        list_commands = [
            ...
            "{}".format(geom),
            "-s",
            "{}".format(software[software_main.value]),
            "-j",
            "{}".format(job[job_main.value]),
            "-m",
            "{}".format(method[method_btn.value]),
            "-c",
            "{}".format(charge.value),
            "-mult",
            "{}".format(mult.value),
    ...
    post_calc = {
        ...
        "Time-dependent calculation": "td",
        "Thermochemistry calculation": "thermo",
        "Optimized energy": "opt",
        "IR plot": "ir",
    }
    post_btn = pn.widgets.Select(
        name="Job Selection",
        value="Frequency calculation",
        options=[
            "Frequency calculation",
    ...
    post_software_main = pn.widgets.Select(
        name="Software selections for post calculations",
        value="Orca",
        options=["Orca", "Dcdftbmd", "Quantum Espresso"],
    )
    post_software = {"Orca": "orca", "Dcdftbmd": "dcdftb", "Quantum Espresso": "qe"}
    # Post CMMDE method options
    post_method_btn = pn.widgets.Select(
        name="Method selections for post calculations",
        value="GFN2-xTB",
        options=[
            "GFN2-xTB",
            "GFN1-xTB",
    ...
    def post_calculation(event):
        ...
        # terminal.clear()
        if (
            post_calc[post_btn.value] == "thermo"
            and post_software[post_software_main.value] == "orca"
        ):
            # Folder = workdir + "/" + id_input.value + "/" + Molecule_input.value
            # os.chdir(Folder)
            ...
            cmd = subprocess.run(
                [
                    "cmmdepost.py",
                    "-j",
                    "{}".format(post_calc[post_btn.value]),
                    "-s",
                    "{}".format(post_software[post_software_main.value]),
                ],
                capture_output=True,
                text=True,
            )
            ...
            cmd = subprocess.run(
                [
                    ...
                    "{}".format(post_software[post_software_main.value]),
                    "-m",
                    "{}".format(post_method[post_method_btn.value]),
                ],
                capture_output=True,
                text=True,
            )
            ...
            list_commands = [
                "cmmde.py",
                "-i",
                "{}/cmmd.xyz".format(Folder + "/" + job[job_main.value]),
                "-s",
                "{}".format(post_software[post_software_main.value]),
                "-j",
                "{}".format(post_calc[post_btn.value]),
    ...
    def calc_progress(event):
        ...
        os.chdir(Folder)
        cmd = subprocess.run(
            ["tail", "-n", "10", "cmmd.out"], capture_output=True, text=True
        )
        TextArea.value = TextArea.value + "\n" + cmd.stdout

    Checkcalc_btn_main = pn.widgets.Button(
        name="Check calculation", button_type="primary"
    )
    ...
    Checkcalc_btn_post = pn.widgets.Button(
        name="Check post-calculation", button_type="primary"
    )
    Checkcalc_btn_post.on_click(calc_progress_post)
    # Slab Builder
    hkl_input = pn.widgets.TextInput(
        name="Miller index (hkl)", placeholder="Example: 100"
    )
    ...
    download_spec_raw = pn.widgets.FileDownload(
        file="IR_fit.dat".format(
            workdir + "/" + id_input.value + "/" + Molecule_input.value + "/" + "ir"
        ),
        filename="IR_fit.dat",
    )
    download_opt_plot = pn.widgets.FileDownload(
        file="optimized.pdf", filename="optimized.pdf"
    )
    download_opt_raw = pn.widgets.FileDownload(
        file="optimized.dat", filename="optimized.dat"
    )
    ...
    def slab_builder(event):
        ...
        cmd = subprocess.run(
            ["echo", "({})-surface construction done!".format(hkl)],
            capture_output=True,
            text=True,
        )
        TextArea.value = TextArea.value + "\n" + cmd.stdout
    ...
    def SolutionBuilder(event):
        ...
        os.chdir("../")
        cmd = subprocess.run(
            ["cmmde.py", "-s", "gromacs", "-mt", "{}".format(solute_folder)]
        )
    ...
    xyzviewer = Py3DMol(
        xyzview, height=400, sizing_mode="stretch_width", name="CMMDE viewer"
    )
    ...
    def visualize(event):
        ...
        with open("cmmd.xyz", "r") as f:
            xyz = f.read()
        xyzview.addModel(xyz, "xyz")
        xyzview.setStyle({"stick": {}, "sphere": {"scale": 0.30}}, viewer=(0, 0))
        xyzview.zoomTo()
        xyzviewer.object = xyzview
    ...
    return pn.template.MaterialTemplate(
        ...
        sidebar=[
            pn.Card(id_input, title="User Information", collapsed=True),
            pn.Card(
                Molecule_input,
                charge,
                mult,
                checkdir_btn,
                TextWarning,
                pn.Card(FileInput, title="Upload molecule", collapsed=True),
            ...
            pn.Card(
                ...
                materialdir_btn,
                TextWarning,
                pn.Card(Material_upload, title="Unit cell", collapsed=True),
                hkl_input,
                size_input,
                layer_input,
                pn.Row(slabbuilder_btn, visual_btn),
                pn.Row(download_slab_xyz, download_slab),
            ...
            pn.Card(
                ...
                dispersion_cor,
                solvent,
                pn.Row(Run_btn, Progress_btn),
                pn.Row(Checkcalc_btn_main, visual_btn),
                download_opt,
            ...
            pn.Card(
                ...
                dispersion_cor,
                solvent,
                pn.Row(runpost_btn, Progress_btn),
                pn.Row(Checkcalc_btn_post),
                pn.Row(download_spec_raw, download_spec_plot),
                pn.Row(download_opt_raw, download_opt_plot),
