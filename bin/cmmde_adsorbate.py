...
parser = argparse.ArgumentParser(
    description="Build possible adsorbate-adsorbent interactions for the surface reaction"
)
...
parser.add_argument(
    ...
    "--height",
    type=float,
    help="height criteria for selection of surface sites",
)
parser.add_argument(
    "-d",
    "--distance",
    type=float,
    help="distance from the coordinating ensemble of atoms along the miller index for the site (i. e. the distance from the slab itself)",
)
parser.add_argument(
    "-pi",
    "--put_inside",
    default=True,
    type=bool,
    help="whether to put the site inside the cell ",
)
parser.add_argument(
    "-sr", "--symm_reduce", type=float, default=0.1, help="symmetry reduction threshold"
)
parser.add_argument(
    "-nr", "--near_reduce", type=float, default=0.1, help="near reduction threshold"
)
parser.add_argument(
    "-noh",
    "--no_obtuse_hollow",
    type=bool,
    default=True,
    help="flag to indicate whether to include obtuse triangular ensembles in hollow sites",
)
parser.add_argument(
    "-sel",
    "--selective_dynamics",
    type=bool,
    default=True,
    help="create the selective dynamics options for VASP calculations",
)
parser.add_argument(
    "-dyn",
    "--dyn",
    type=float,
    default=3.50,
    help="Height for dynamical atoms selections",
)
parser.add_argument(
    "-t", "--type", type=str, help="Type of adsorption site based on the list"
)
parser.add_argument("-ad", "--ad", type=str, help="Structure of adsorbate from file")
parser.add_argument(
    "-all", "--all", type=bool, default=False, help="Print all possible structures"
)
...
adsorbent = pa.AdsorbateSiteFinder(
    slab_coord, height=opt.dyn, selective_dynamics=opt.selective_dynamics
)
...
if opt.all == False:
    sites = check_site.find_adsorption_sites(
        distance=opt.distance,
        put_inside=opt.put_inside,
        symm_reduce=opt.symm_reduce,
        near_reduce=opt.near_reduce,
        positions=[opt.type],
        no_obtuse_hollow=opt.no_obtuse_hollow,
    )
...
# The following if in case you want to generate all possible adsorption sites.
    ...
    for site in hollow_coord:
        ...
        sorted_ads = adsorbed.get_sorted_structure(
            key=lambda x: element.index(str(x.specie))
        )
    ...
    print(
        "hollow:{}, bridge:{}, ontop:{}".format(
            len(hollow_coord), len(bridge_coord), len(ontop_coord)
        )
    )
