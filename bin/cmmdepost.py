...
parser.add_argument(
    ...
    type=str,
    default="None",
    help="Jenis perhitungan yang dilakukan. Pilihan: sp, opt, thermo, ir, reax, msd_com, msd, msd_fit rdf, td.",
)
parser.add_argument(
    "-m",
    "--method",
    type=str,
    default="XTB2",
    help="Metode yang digunakan dalam perhitungan.",
)
parser.add_argument(
    "-i",
    "--input",
    type=str,
    default="geom.xyz",
    help="Koordinat awal dalam format .xyz (hanya diperlukan untuk program DCDFTBMD) saat optimasi geometri.",
)
parser.add_argument(
    "-s",
    "--software",
    type=str,
    default="orca",
    help="Jenis software yang digunakan. Pilihan: orca, dcdftb, dftb+.",
)
parser.add_argument(
    "-irx", "--inputreaction", type=str, default="None", help="Input reaksi kimia"
)
parser.add_argument(
    "-t",
    "--temp",
    type=float,
    default=298.15,
    help="Suhu yang digunakan dalam perhitungan frekuensi dan termokimia",
)
parser.add_argument(
    "-traj",
    "--traject",
    type=str,
    default="traject.xyz",
    help="File trayektori dinamika molekul dalam format .xyz",
)
parser.add_argument(
    "-l", "--latt", type=str, default="cmmd.in", help="Ukuran rusuk sel satuan."
)
parser.add_argument(
    "-start",
    "--start",
    type=int,
    default=0,
    help="Titik acuan dalam bilangan bulat. Default = 0.",
)
parser.add_argument(
    "-end", "--end", type=int, default=-1, help="Titik ahir pembacaan trayektori"
)
parser.add_argument(
    "-msd",
    "--msd",
    type=str,
    default="msd.out",
    help="File berisikan MSD keluaran cmmdepost.py -j msdcom.",
)
parser.add_argument(
    "-dt",
    "--dt",
    type=float,
    default=0.5,
    help="Time step yang didefinisikan selama simulasi berlangsung.",
)
parser.add_argument(
    "-n",
    "--noheader",
    action="store_true",
    help="Mendefinisikan apakah file mengandung header atau tidak. Default file msd.out mengandung header berupa #.",
)
parser.add_argument(
    "-g",
    "--groups",
    type=str,
    help="Molekul didefinisikan dengan Nama*JumlahAtom*JumlahMolekul. Contoh NAP*1*2:DMF*1*20.",
)
parser.add_argument(
    "-r", "--range", default=10.0, type=float, help="Jarak terjauh pengukuran RDF"
)
parser.add_argument(
    "-p",
    "--pair",
    type=str,
    help='Pasangan atom, misalnya "O-O" untuk pasangan atom O-O',
)
parser.add_argument(
    "-res",
    "--resolution",
    default=0.01,
    type=float,
    help="Resolusi untuk plot histogram atau RDF",
)
# Metadinamika
parser.add_argument(
    "-metafreq",
...
parser.add_argument(
    "-fesend", "--fesend", type=float, default=1, help="Titik maksimum CV. Default = 1."
)
parser.add_argument(
...
parser.add_argument(
    "-ngaus", "--ngaus", type=int, help="Jumlah Gaussian yang didepositkan"
)
parser.add_argument(
    "-nlig", "--nligands", type=int, help="Jumlah ligan yang akan didocking."
)
parser.add_argument(
    "-nr", "--nroots", type=int, default=5, help="Jumlah keadaan tereksitasi."
)
parser.add_argument(
    "-pts", "--points", type=int, default=300, help="Jumlah titik untuk plot UV."
)
# Analisis DOS
parser.add_argument(
    "-pdos", "--pdos", type=str, help="File DOS parsial hasil perhitungan DFTB+"
)
parser.add_argument(
    "-xlabel", "--xlabel", type=str, default="x", help="Label sumbu-x dalam sebuah plot"
)
parser.add_argument(
    "-ylabel", "--ylabel", type=str, default="y", help="Label sumbu-y dalam sebuah plot"
)
parser.add_argument("-file", "--file", type=str, help="Nama file yang akan diplot")
parser.add_argument(
    "-np", "--nproc", type=int, default=1, help="Jumlah prosesor yang digunakan."
)
parser.add_argument(
    "-grid",
    "--grid",
    type=str,
    default="FINE",
    help="Jenis grid yang akan dibangun untuk perhitungan NCI. Pilihan: FINE (default), COARSE, dan ULTRAFINE.",
)
...
parser.add_argument(
    ...
    type=float,
    default=-9.0,
    help="Batas minimum nilai keadaan elektronik pada DOS.",
)
parser.add_argument(
    "-emax",
    "--emax",
    type=float,
    default=16.0,
    help="Batas maksimum nilai keadaan elektronik pada DOS.",
)
...
if (
    opt.job == "sp"
    and opt.software == "orca"
    and not (opt.method == "XTB" or opt.method == "XTB2")
):
    if os.path.isfile("cmmd.out"):
        ...
        print(
            "Energi total molekul: {} Hartree = {:.2f} kJ/mol".format(
                Energy, Energy * Ha2kj
            )
        )
    else:
    ...
    if os.path.isfile("cmmd.out"):
        ...
        print(
            "Norm Gradien: {} Hartree/a0 = {:.2f} kJ/mol/a0".format(
                Gradient, Gradient * Ha2kj
            )
        )
...
if opt.job == "opt" and opt.software == "orca":
    ...
    if os.path.isfile("cmmd.out"):
        ...
        print(
            "Total energi elektronik = {} Hartree = {} kJ/mol".format(
                TotalEnergy[-1], TotalEnergy[-1] * Ha2kj
            )
        )
        if (
            method != "XTB1"
...
plot 'optimized.dat' using 1:2 notitle with linespoints linestyle 1""",
                file=fout,
            )
...
if opt.job == "ir" and opt.software == "orca":
    ...
    def gaussband(x, band, strength, stdev):
        bandshape = (
            1.3062974e8
            * (strength / (1e7 / stdev))
            * np.exp(-((((1.0 / x) - (1.0 / band)) / (1.0 / stdev)) ** 2))
        )
        return bandshape
...
if opt.job == "thermo" and opt.software == "orca":
    ...
    if os.path.isfile("cmmd.out"):
        ...
        print(
            "Energi elektronik = {} Hartree = {} kJ/mol".format(E_elec, E_elec * Ha2kj)
        )
        print(
            "Zero point energy (ZPE) = {} Hartree = {} kJ/mol".format(ZPE, ZPE * Ha2kj)
        )
        print(
            "Koreksi termal vibrasi = {} Hartree = {} kJ/mol".format(Evib, Evib * Ha2kj)
        )
        print(
            "Koreksi termal rotasi = {} Hartree = {} kJ/mol".format(Erot, Erot * Ha2kj)
        )
        print(
            "Koreksi termal translasi = {} Hartree = {} kJ/mol".format(
                Etrans, Etrans * Ha2kj
            )
        )
        print(
            "Total koreksi termal = {} Hartree = {} kJ/mol".format(
                Etrans + Evib + Erot, (Etrans + Evib + Erot) * Ha2kj
            )
        )
        print("")
        ...
        print(
            "Energi bebas Gibbs (G) = {} Hartree = {} kJ/mol".format(
                Gibbs, Gibbs * Ha2kj
            )
        )
...
if (
    opt.job == "gap"
    and ("XTB" not in opt.method and "XTB2" not in opt.method)
    and opt.software == "orca"
):
    with open("cmmd.out", "r") as f:
        lines = f.readlines()
...
if (
    opt.job == "gap"
    and ("XTB" in opt.method or "XTB2" in opt.method)
    and opt.software == "orca"
):
    E_HOMO = []
    E_LUMO = []
...
if opt.job == "fes" and opt.software == "dcdftb":
    ...
    with open("fes.dat", "r") as f:
        ...
        for index, line in enumerate(lines):
            if (
                "FREE ENERGY SURFACE CONSISTING OF     {} GAUSSIANS".format(opt.ngaus)
                in line
            ):
...
if opt.job == "cell":
    os.system(
        "aflow --data < {}".format(opt.input)
    )  # Untuk menggunakan fitur ini, install aflow, wget http://aflow.org/install-aflow/install-aflow.sh
...
plot 'cmmd.dat' u 1:2:1 w p lw 6 palette t '' """,
            file=f,
        )
...
if "dos" in opt.job and opt.software == "qe":
 ...
 emax = {},
 /""".format(
                opt.outdir, opt.emin, opt.emax
            ),
            file=f,
        )
    os.system(
        "$QE_DOS_COMMAND < cmmd.in > cmmd.out"
    )  # Hasil perhitungan berupa cmmd.dos
...
cd $PWD
$QE_PP_COMMAND < cmmd.in > cmmd.out""",
            file=fout,
        )
