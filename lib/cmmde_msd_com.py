...
def msd_com(groups, traject, start, latt, dt):
    ...
    if groups_nat != nat:
        raise ValueError(
            "Salah dalam mendefinisikan molekul! Jumlah atom tidak konsisten."
        )

    print("Kelompok:")
    for k, nat, nmole in index_groups:
        print(
            "    Nama: {}, Jumlah atom dalam molekul: {}, Jumlah molekul dalam sistem: {}".format(
                k, nat, nmole
            )
        )
    print("")
    ...
    print(
        "#Time(ps) ",
        format_str.format(*[k for k, nat, nmole in index_groups]),
        file=fout,
    )
        ...
        while True:
            try:
                ...
                for group in index_groups:
                    ...
                    for mole_index in range(group[2]):
                        ...
                        if len(local_indexs) == 1:
                            ...
                            com_local = (
                                atom1.specie.data["Atomic mass"] * atom1.frac_coords
                            )
                            total_mass = atom1.specie.data["Atomic mass"]
                            for at2 in local_indexs[1:]:
                                atom2 = struct[at2]
                                dis_, image_ = atom1.distance_and_image(atom2)
                                com_local += atom2.specie.data["Atomic mass"] * (
                                    atom2.frac_coords + image_
                                )
                ...
                if step == 1:
                    ...
                    for first_com, last_com, cur_com, image_last in zip(
                        first_coms, last_coms, com_step, image_lasts
                    ):
                        image_current = struct.lattice.get_distance_and_image(
                            last_com, cur_com
                        )[1]
                        image_final = image_last + image_current
                        final_dis = struct.lattice.get_distance_and_image(
                            first_com, cur_com, jimage=image_final
                        )[0]
...
def msd_fit(msd, noheader, start):
    ...
    with open(data_file, "r") as f:
        for line in f:
            ...
            if ncol == -1:
                ...
                if ncol != (len(arr) - 1):
                    raise ValueError(
                        "The number of columns of data file is not consistent!"
                    )
    ...
    for col in range(ncol):
        ...
        print(
            " Koefisien Difusi = {:.2f} A^2/ps = {:.2e} m^2/s".format(
                diff_coeff, diff_coeff * 1.0e-8
            )
        )
