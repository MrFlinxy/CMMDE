...
parser = argparse.ArgumentParser(
    description="Compute Diffusion Coeffients from MSD data (nxy format)"
)
...
for col in range(ncol):
    ...
    print(
        " Diffusion coefficient (1/6): {:16.10F} A^2/ps, {:17.12E} m^2/s".format(
            diff_coeff, diff_coeff * 1.0e-8
        )
    )
    diff_coeff = ols_result.params[1] / 2.0
    print(
        " Diffusion coefficient (1/2): {:16.10F} A^2/ps, {:17.12E} m^2/s".format(
            diff_coeff, diff_coeff * 1.0e-8
        )
    )
    diff_coeff = ols_result.params[1] / 4.0
    print(
        " Diffusion coefficient (1/4): {:16.10F} A^2/ps, {:17.12E} m^2/s".format(
            diff_coeff, diff_coeff * 1.0e-8
        )
    )
    print("*" * 80)
