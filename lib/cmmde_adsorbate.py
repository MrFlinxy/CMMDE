...
def bcc111(symbol, size, a=None, vacuum=None, orthogonal=False, periodic=False):
    """BCC(111) surface.
...
def hcp0001(
    symbol, size, a=None, c=None, vacuum=None, orthogonal=False, periodic=False
):
    """HCP(0001) surface.
...
def hcp10m10(
    symbol, size, a=None, c=None, vacuum=None, orthogonal=True, periodic=False
):
    """HCP(10m10) surface.
...
def add_adsorbate(slab, adsorbate, height, position=(0, 0), offset=None, mol_index=0):
    ...
    if isinstance(position, str):
        ...
        if "sites" not in info:
            raise TypeError(
                "If the atoms are not made by an "
                + "ase.build function, "
...
def add_vacuum(atoms, vacuum):
    ...
    costheta = np.dot(normal, uc[2]) / np.sqrt(
        np.dot(normal, normal) * np.dot(uc[2], uc[2])
    )
...
def _surface(symbol, structure, face, size, a, c, vacuum, periodic, orthogonal=True):
    ...
    if a is None:
        ...
        if sym != structure:
            raise ValueError(
                "Can't guess lattice constant for %s-%s!" % (structure, symbol)
            )
    ...
    if surf == "fcc100":
        ...
        positions[-2::-2, ..., :2] += 0.5
        sites.update(
            {"hollow": (0.5, 0.5), "longbridge": (0.5, 0), "shortbridge": (0, 0.5)}
        )
    elif surf == "bcc100":
        cell = (1.0, 1.0, 0.5)
        positions[-2::-2, ..., :2] += 0.5
        ...
        if orthogonal and size[1] % 2 == 1:
            raise ValueError(
                ("Can't make orthorhombic cell with size=%r.  " % (tuple(size),))
        ...
        if not orthogonal:
            cell = np.array(
                [(cell[0], 0, 0), (cell[0] / 2, cell[1], 0), (0, 0, cell[2])]
            )
...
def fcc211(symbol, size, a=None, vacuum=None, orthogonal=True):
    ...
    orders = [
        (atom.index, round(atom.x, 3), round(atom.y, 3), -round(atom.z, 3), atom.index)
        for atom in atoms
    ]
