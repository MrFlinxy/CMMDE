def orca(
    ...
    if "neb-ci" in job:
        method += " NEB-CI"
    if "freq" in job and (
        "XTB2" not in method and "XTB" not in method and "XTBFF" not in method
    ):
        method += " freq"
    if disp != "None":
        method += " {}".format(disp)
...
""".format(
                    irciter, printlevel, inithess, hessfile, grid, finalgrid
                ),
                file=f,
            )
    return
