...
def addcharge(geom, chargetype):
    ...
    filename = arr[0]
    os.system(
        "obabel {} --partialcharge {} -O {}.mol2".format(geom, chargetype, filename)
    )
...
def translig(ligand):
    ...
    os.system(
        "$DOCK_DIR/transform.py {}_center.xyz -tx {} -ty {} -tz {} > {}_trans.xyz".format(
            filename, center_x, center_y, center_z, filename
        )
    )
...
def multiopt(nligands):
    for i in range(1, nligands + 1):
        ...
        os.system(
            "cmmde.py -s dcdftb -j opt -i lig{}.xyz -m DFTB3 -disp D3BJ -np 2 -solvent water".format(
                i
            )
        )
        os.chdir("../")
...
def multiflexdock(nligands, chargetype):
    for i in range(1, nligands + 1):
        ...
        os.system(
            "cmmde.py -s dock -j addcharge -i cmmd.pdb -chargetype {}".format(
                chargetype
            )
        )
