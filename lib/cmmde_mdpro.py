...
def ligprep(ligand, charge):
    ...
    with open("temp_{}_h.pdb".format(filename), "r") as oldpdb, open(
        "{}_h.pdb".format(filename), "w"
    ) as newpdb:
        for line in oldpdb:
    ...
    os.system(
        "antechamber -j 5 -at gaff -dr no -i {}_h.pdb -fi pdb -o {}_h.mol2 -fo mol2 -c gas -s 2 -nc {}".format(
            filename, filename, charge
        )
    )
